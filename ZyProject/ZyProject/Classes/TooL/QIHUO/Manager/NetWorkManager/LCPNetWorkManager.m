...
@implementation LCPNetWorkManager
...
+ (AFHTTPSessionManager *)shareManager {
    ...
    dispatch_once(&onceToken, ^{
        manager = [LCPNetWorkManager managerWithBaseURL:LCP_BASE_URL sessionConfiguration:YES];
    });
...
+ (void)setHeaderWithManager:(AFHTTPSessionManager *)manager headers:(NSDictionary *)headers PathUrl:(NSString *)pathUrl{
    ...
    headers = [LCPNetWorkManager configHeaderWithHeaders:headers];
...
+(void)uploadPicWithUrl:(NSString *)url parameters:(NSDictionary *)parameters success:(HTTPSuccessBlock)successBlock failure:(HTTPFailureBlock)failureBlock{
    AFHTTPSessionManager *manager = [LCPNetWorkManager managerWithBaseURL:@"http://image.yysc.online" sessionConfiguration:NO];
    [LCPNetWorkPOST POSTFormDataWithParams:parameters pathurl:url success:^(BOOL success, id result) {
        successBlock(success,result);
    } failure:^(BOOL failuer, NSError *error) {
        failureBlock(failuer,error);
    } manager:manager];
...
//    }
    ...
    AFHTTPSessionManager *manager = [LCPNetWorkManager shareManager];
    
    [LCPNetWorkManager setHeaderWithManager:manager headers:headerParam PathUrl:pathUrl];
    ...
    switch (requestMethod) {
        case kHttpRequestTypeFormPost:{
            [LCPNetWorkPOST POSTFormDataWithParams:parameters pathurl:newPathURl success:successBlock failure:failureBlock manager:manager];
        }break;
        case kHttpRequestTypePost:{
            
            [LCPNetWorkPOST POSTDataWithParams:parameters pathurl:newPathURl success:successBlock failure:failureBlock manager:manager];
        }break;
        case kHttpRequestTypeGet:{
            [LCPNetWorkGET GETWithParameter:parameters pathurl:newPathURl success:successBlock failure:failureBlock manager:manager];
            //            //数据缓存处理
        }break;
        case kHttpRequestTypeDelete: {
            [LCPNetWorkDELETE DELETEWithParams:parameters pathurl:newPathURl success:successBlock failure:failureBlock manager:manager];
        }break;
        case kHttpRequestTypePut: {
            [LCPNetWorkPUT PUTWithParams:parameters pathurl:newPathURl success:successBlock failure:failureBlock manager:manager];
            
        }
            break;
        case kHttpRequestTypePatch:
        {
            [LCPNetWorkPUT PATCHWithParams:parameters pathurl:newPathURl success:successBlock failure:failureBlock manager:manager];
        }
            break;
        case kHttpRequestTypeJSONPost:
        {
            [LCPNetWorkPOST POSTJSONWithParams:parameters pathurl:newPathURl success:successBlock failure:failureBlock manager:manager];
        }
            break;
        default:break;
    }
...
+ (void)getDownLoadWithPathUrl:(NSString *)pathUrl parameters:(NSDictionary *)parameters success:(HTTPSuccessBlock)successBlock failure:(HTTPFailureBlock)failureBlock
{
    ...
    CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault,
                                                              ...
                                                              NULL,
                                                              kCFStringEncodingUTF8));
    [LCPNetWorkGET GETDownLoadParameter:parameters pathurl:pathUrl success:successBlock failure:failureBlock manager:manager];
